...
class HttpHandlers {
	 ...
	 */
	IFunction enrichFunc(RequestMatchable requestMatcher, IFunction func, Object... params) {
		...
		for (Class c : targetClasses) {
			...
			Constructor constructor = ReflectionUtils.findConstructor(c, new Class[0]);
			...
			for (Method method : annotatedMethods) {
				...
				if (actualMatcher.isMatched(requestMapper)) {
					Object instance = ReflectionUtils.newInstance(constructor);
